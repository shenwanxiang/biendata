...
from molmaps.logtools import print_info, print_warn, print_error
from molmaps.matrixopt import Scatter2Grid, Scatter2Array 
from molmaps import vismap
...
colormaps = {'descriptor': '#ff6a00', 'fingerprint':'#a700ff', 'NaN': '#000000' }
...
class MolMap(Base):
    ...
    def __init__(self, 
        ...
        dist_matrix = load_config('feature', metric)
        ...
        bitsinfo = load_config(ftype, 'bitsinfo')  
    ...
    def transform(self, 
        ...
        parameters
        --------------------
        arr: the feature arrays for one compounds
    ...
    def batch_transform(self, 
        ...
        res = P(delayed(self.transform)(smiles, 
                                        scale,
                                        scale_method) for smiles in tqdm(arr_list, ascii=True)) 
        X = np.stack(res) 
