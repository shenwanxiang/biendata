...
class Reg_EarlyStoppingAndPerformance(tf.keras.callbacks.Callback):
    ...
    def __init__(self, train_data, valid_data, n_conformers,  MASK = -1, patience=5, criteria = 'val_loss'):
    ...
    def rmse(self, y_true, y_pred):
        ...
        for i in range(N_classes):
            ...
            y1 = y_true_one_class[mask]
            y2 = y_pred_one_class[mask]
            y1 = pd.Series(y1).rolling(self.n_conformers).mean().dropna().tolist()
            y2 = pd.Series(y2).rolling(self.n_conformers).mean().dropna().tolist()
...
class CLA_EarlyStoppingAndPerformance(tf.keras.callbacks.Callback):
    ...
    def __init__(self, train_data, valid_data, n_conformers, MASK = -1, patience=5, criteria = 'val_loss', metric = 'ROC'):
    ...
    def roc_auc(self, y_true, y_pred):
        ...
        for i in range(N_classes):
            ...
            try:
                if self.metric == 'ROC':
                    auc = roc_auc_score(y1, y2) #ROC_AUC
                    
                elif self.metric == 'PRC':                  
                    auc = prc_auc_score(y1, y2) #PRC_AUC                    
                    
                elif self.metric == 'ACC':
                    auc = accuracy_score(y1, np.round(y2)) #ACC
            except:
                auc = np.nan
            aucs.append(auc)
        return aucs  
